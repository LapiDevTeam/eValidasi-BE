...
const repo = require('../../repositories/rpm-calibration.repository');
const formula = require('./rpmFormula.service');
...
function normalizePoints(points = []) {
  return points.map((point, index) => ({
    ...
    correction: toNumberOrNull(point.correction) ?? 0,
...
async function calculate(sessionId, changedBy = null) {
  ...
  try {
    ...
    if (!points.length || !readings.length) {
      throw httpError('No RPM workbook readings found.', 422, [{ field: 'points', message: 'Isi data pembacaan RPM terlebih dahulu.' }]);
    }
...
async function listDaCandidates(filters) {
  return repo.listRpmDaCandidates(filters);
}

async function resolveQaCandidate(session, explicitQaId, transaction) {
  if (explicitQaId) {
    const matches = await repo.listRpmDaCandidates({ qa_id: explicitQaId }, transaction);
  ...
  if (sessionQaId) {
    const matches = await repo.listRpmDaCandidates({ qa_id: sessionQaId }, transaction);
    if (matches.length === 1) return matches[0];
  }
  const instrumentCode = String(session?.instrument_code || '').trim();
  if (instrumentCode) {
    const candidates = await repo.listRpmDaCandidates({ instrument_code: instrumentCode }, transaction);
  ...
  try {
    ...
    if (!results.length) {
      throw httpError(
        ...
        422,
        [{ field: 'status', message: 'Tabel hasil (rpm_results) masih kosong.' }]
      );
    }
    ...
    if (!idNoSertifikat) {
      const certCode = getCertificateTypeCode(qaCandidate.Parameter_Sertifikasi) || 'RP';
    ...
    if (!header) {
      const inserted = await repo.createSertifikatBagianDraftFromRpmDa(qaId, idNoSertifikat, actor, delegatedActor, transaction);
    ...
    await repo.updateSertifikatBagianHeader({
      ...
      metode_kalibrasi: options.metode_kalibrasi ?? session.metode_kalibrasi ?? 'Kalibrasi RPM - Workbook',
    ...
    await repo.insertAuditLog({
      session_id: sessionId,
      entity_name: 'rpm_publish_sertifikat',
      entity_id: null,
      action_type: 'UPDATE',
      old_value: null,
