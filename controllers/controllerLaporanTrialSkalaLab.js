...
class ControllerLaporanTrialSkalaLab {
  ...
  static async getLaporanTrialSkalaLabDetails(req, res, next) {
    try {
      ...
      const hasilPengamatan = await t_LTS_hasilPengamatan.findAll({
        where: {
          LaporanTrialSkalaLabID: id,
        },
      });
  ...
  static async handleSaveHasilPengamatan(req, res) {
    const transaction = await sequelize.transaction();
    try {
      const { data } = req.body;
      ...
      const prevKriteria = await t_LTS_hasilPengamatan.findAll({
        where: {
          LaporanTrialSkalaLabID: id,
        },
        order: [["id", "ASC"]],
      ...
      await Promise.all(
        data?.map(async (newItem) => {
          ...
          if (!newItem?.id) {
            const created = await t_LTS_hasilPengamatan.create(
              {
                waktuPengamatan: newItem?.waktuPengamatan || "",
      ...
      const newData = await t_LTS_hasilPengamatan.findAll({
        where: {
          LaporanTrialSkalaLabID: +id,
        },
        order: [["id", "ASC"]],
