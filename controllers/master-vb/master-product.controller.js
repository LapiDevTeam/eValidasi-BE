...
class MasterProductController {
  static async fetchProduct(req, res, next) {
    try {
      ...
      // Add jenis_kemasan from m_Product_template by PK_ID
      const sqlCode = `
        select
          A.Product_ID,
          Product_Name,
  ...
  static async fetchProductById(req, res, next) {
    try {
      const { productID, isTemplate = 1 } = req.query;

      if (!productID) {
        return res.status(400).json({ message: 'productID is required!' });
      }

      // Choose table based on isTemplate
      const tableName = isTemplate == 1 ? 'm_Product_template' : 'm_Product';

      // Build query (select all columns for completeness)
      const sqlCode = `
      SELECT
        Product_ID,
        Product_Init,
        ...
        Product_Batchsize,
        Product_Location,
        Product_GroupSediaan,
        Product_GroupTargetRealisasi,
        Product_SalesName,
        Product_Status,
        Product_SalesHNA,
        Product_ShortName,
        ...
        Product_isSuplemen,
        Product_LabelQA,
        Product_RetainBigSuperUnit,
        Product_RetainKonversiBig,
        Product_RetainBigUnit,
        Product_RetainKonversi,
        Product_RetainSmallUnit,
        Product_SediaanPlanning,
        Product_Owner,
        product_import,
        Product_KonversiProduksiToSales,
        Product_bahanAktif,
        Product_PN_HK_PProcessing,
        Product_PN_HK_PKSekunder,
        Product_PN_HK_Produksi,
        Product_PN_HK_PPIProduksi,
        Product_versiBPOM,
        Product_NotPPI,
        Product_BentukSediaan,
        Product_Kemasan,
      ...
      ${isTemplate == 1 ? "AND ISNULL(Product_Periode, '') = ''" : ''}
    `;

      const [result] = await sequelizeMSQL.query(sqlCode, {
        type: QueryTypes.SELECT,
  ...
  static async addNewProduct(req, res, next) {
    try {
      ...
      if (!productID) {
        ...
        if (jenis_kemasan === 'BFS' && productRuangLingkup !== '04') {
          queryGetProduct = `
            SELECT TOP 1 isnull(product_id, '') as Product_ID
            FROM m_product_auto_number
            WHERE pk_id > (
      ...
      const insertProductSQL = `
        insert into m_Product_template (jenis_kemasan, jenis_prod, Kategori_prod, PK_ID, Product_ID, Product_Init, Product_Name, Product_Category, Product_Currency, Product_HPP, Product_HNA, Product_HTollIN, Product_HTollINFee, Product_VolumeInBox, Product_VolumeInBigBox, Product_Unit, Product_Type, Product_IntermediateID, jenis_kemasan, isActive, User_ID, Delegated_To, Process_Date, Product_BatchSize, Product_owner, Product_bahanAktif, product_bentuksediaan, product_kemasan, product_dosis, product_ruanglingkup, product_status, CDOB_01, CDOB_02, CDOB_03, product_import)
  ...
  static async approveProduct(req, res, next) {
    ...
    `;
      console.log({ sSQL5 });
      let sSQL6 = `
      INSERT INTO m_Product_template (jenis_kemasan, Kategori_prod, Product_ID, Product_Init, Product_Name, jenis_prod, Product_Category, Product_Currency, Product_HPP, Product_HNA, Product_HTollin, Product_HTollInFee,
        Product_VolumeInBox, Product_VolumeInBigBox, Product_Unit, Product_Type, Product_IntermediateID, isActive, User_ID, Delegated_To, Process_Date,
        Product_Sediaan, Product_ExpTime, Product_SalesID, Product_isPPA, Product_isPseudo, Product_isBuy, Product_Batchsize, Product_Location,
        Product_GroupSediaan, Product_GroupTargetRealisasi, Product_SalesName, Product_Status, Product_SalesHNA, Product_ShortName, Product_SalesUnit,
        Product_isSuplemen, Product_LabelQA, Product_RetainBigSuperUnit, Product_RetainKonversiBig, Product_RetainBigUnit, Product_RetainKonversi,
    ...
    try {
      ...
      for (let i = 0; i < products.length; i++) {
        ...
        products[i].bahan_aktif_detail = bahanAktifRows.map(
          (row) =>
            `- ${row.Product_BahanAktif?.toString().trim().replace(/\s+/g, ' ') || ''} (${
              row.Product_Dosis?.toString().trim().replace(/\s+/g, ' ') || ''
            })`
        );
      }
  ...
  static async deleteProductBAK(req, res, next) {
    ...
    try {
      ...
      if (formulaResult.length > 0) {
        return res
          .status(400)
          .json({ message: 'Master product sudah ada di formula, tidak bisa di hapus, harap hapus formula jika ingin hapus produk' });
      }
