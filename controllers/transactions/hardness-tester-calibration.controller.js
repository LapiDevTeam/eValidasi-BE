...
const SESSION_TABLE = 'dbo.T_Kalibrasi_HardnessTester_Workbook_Session';
...
function buildHardnessTesterResultRows(calculationResult = {}, explicitRows = []) {
  const sourceRows = [];

  if (Array.isArray(explicitRows) && explicitRows.length) {
    sourceRows.push(...explicitRows);
  } else {
    const labels = {
      hardness: 'Hardness',
      thickness: 'Thickness',
    ...
    for (const key of activeKeys) {
      ...
      for (const point of test?.points || []) {
        sourceRows.push({
          ...
          pembacaanAlat: key === 'hardness' ? point.result : point.avgUut,
          pembacaanStandar: key === 'hardness' ? point.convertedStandard : point.avgStd,
          error: key === 'hardness' ? point.error : point.avgError,
          ketidakpastian: key === 'hardness' ? point.expandedUncertainty : null,
        });
      }
    }
  }
  ...
  return sourceRows.map((row, index) => {
    const pembacaanAlat = parseNumberValue(
      row.pembacaanAlat ?? row.Pembacaan_Alat ?? row.avgUut ?? row.result
    );
    const pembacaanStandar = parseNumberValue(
      row.pembacaanStandar ?? row.Pembacaan_standar ?? row.avgStd ?? row.convertedStandard
    );
    const error = parseNumberValue(row.error ?? row.ERROR ?? row.avgError) ??
      (pembacaanAlat === null || pembacaanStandar === null ? null : pembacaanAlat - pembacaanStandar);

    return {
      seqId: Number(row.seqId || row.Seq_ID || index + 1),
      no: Number(row.no || row.seqId || index + 1),
      parameter: textValue(row.parameter || row.Parameter),
      setting: parseNumberValue(row.setting ?? row.Setting),
      unit: textValue(row.unit || row.Unit),
      pembacaanAlat,
      pembacaanStandar,
      error,
      ketidakpastian: parseNumberValue(row.ketidakpastian ?? row.Ketidakpastian),
    };
  });
}
...
async function getNextHardnessTesterCertificateNumber(transaction) {
  const rows = await sequelizeMSQL.query(
    'SELECT dbo.fnGetKal_Ser_HT_No_ID() as ID_No_sertifikat',
    {
      type: Sequelize.QueryTypes.SELECT,
      transaction,
    }
...
async function ensureHardnessTesterCertificateHeader({ qaId, idNoSertifikat, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SELECT
        ...
        Jenis_kalibrasi,
        COALESCE(NULLIF(Parameter_Sertifikasi, ''), 'Hardness Tester') AS parameter_sertifikasi,
        1 AS isSert_Manual,
        GETDATE() AS Tgl,
        Assm_nama_instrumen,
        Assm_No_identitas_Istrumen,
...
async function updateHardnessTesterCertificateHeader({ qaId, idNoSertifikat, header, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SET
        parameter_sertifikasi = COALESCE(NULLIF(parameter_sertifikasi, ''), 'Hardness Tester'),
...
async function replaceHardnessTesterCertificateRows({ qaId, idNoSertifikat, rows, userId, delegatedTo, transaction }) {
  await sequelizeMSQL.query(
    `
...
const listSessions = async (req, res, next) => {
  try {
    ...
    console.error('Error in HardnessTester listSessions:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(404).json({
        success: false,
        message: 'Hardness Tester session table has not been created',
      });
    }
    ...
    console.error('Error in HardnessTester getSession:', error);
    next(error);
  }
};
...
const getPrintData = async (req, res, next) => {
  try {
    ...
    if (!headerResults.length) {
      return res.status(404).json({
        success: false,
        message: 'Hardness Tester sertifikat data not found',
      });
    }

    const workbookPayload = latestSession?.workbookPayload || {};
    const rows = buildHardnessTesterResultRows(latestSession?.calculationResult || {});
    ...
    console.error('Error in HardnessTester getPrintData:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(500).json({
        success: false,
        message: 'Hardness Tester session table has not been created. Run sql/create-hardness-tester-calibration-tables.sql first.',
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Hardness Tester workbook session saved successfully',
      data: session,
    });
  } catch (error) {
    console.error('Error in HardnessTester saveSession:', error);
    next(error);
  }
};
...
const approveSession = async (req, res, next) => {
  try {
    ...
    console.error('Error in HardnessTester approveSession:', error);
    next(error);
  }
};

const generateHardnessTesterSertifikat = async (req, res, next) => {
  ...
  try {
    ...
    if (!qaId) {
      ...
      return res.status(400).json({
        success: false,
        message: 'QA_ID is required to generate Hardness Tester certificate',
      });
    }

    if (!idNoSertifikat) {
      idNoSertifikat = await getNextHardnessTesterCertificateNumber(transaction);
    }
    ...
    if (!idNoSertifikat) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Gagal mengambil nomor otomatis sertifikat Hardness Tester',
      });
    }

    const headerCreated = await ensureHardnessTesterCertificateHeader({
      qaId,
      idNoSertifikat,
      userId: user_id,
      delegatedTo: delegated_to,
    ...
    await updateHardnessTesterCertificateHeader({
      qaId,
      idNoSertifikat,
    ...
    const resultRows = buildHardnessTesterResultRows(
      calculationResult,
      body.certificateRows || []
    );
    await replaceHardnessTesterCertificateRows({
      qaId,
      idNoSertifikat,
      rows: resultRows,
      userId: user_id,
    ...
    return res.status(200).json({
      success: true,
      message: 'Sukses Generate Data Sertifikat Hardness Tester!',
      data: {
        qa_id: qaId,
        id_no_sertifikat: idNoSertifikat,
    ...
    console.error('Error in generateHardnessTesterSertifikat:', error);
    next(error);
  }
};
...
module.exports = {
  ...
  saveSession,
  approveSession,
  generateHardnessTesterSertifikat,
};
