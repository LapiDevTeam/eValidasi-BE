...
function buildTemperatureControlResultRows(calculationResult = {}, explicitRows = []) {
  const sourceRows = Array.isArray(explicitRows) && explicitRows.length
    ? explicitRows
    : Array.isArray(calculationResult?.certificateRows) && calculationResult.certificateRows.length
  ...
  return sourceRows.map((row, index) => {
    ...
    const pembacaanAlat = parseNumberValue(
      row.pembacaanAlat ?? row.Pembacaan_Alat ?? row.indicator ?? selected.indicator
    );
    const pembacaanStandar = parseNumberValue(
      row.pembacaanStandar ?? row.Pembacaan_standar ?? row.reference ?? selected.reference
    );
    const error = parseNumberValue(row.error ?? row.ERROR ?? selected.error) ??
    ...
    return {
      ...
      pembacaanStandar,
      error,
      ketidakpastian: parseNumberValue(
        row.ketidakpastian ?? row.Ketidakpastian ?? uncertainty.expanded
      ),
      unit: textValue(row.unit || calculationResult?.setup?.unit || '°C'),
    };
  });
}
...
async function getNextTemperatureControlCertificateNumber(transaction) {
  const rows = await sequelizeMSQL.query(
    'SELECT dbo.fnGetKal_Ser_TC_No_ID() as ID_No_sertifikat',
    {
      type: Sequelize.QueryTypes.SELECT,
      transaction,
    }
...
async function ensureTemperatureControlCertificateHeader({ qaId, idNoSertifikat, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SELECT
        ...
        Jenis_kalibrasi,
        COALESCE(NULLIF(Parameter_Sertifikasi, ''), 'Temperature Control') AS parameter_sertifikasi,
        1 AS isSert_Manual,
        GETDATE() AS Tgl,
        Assm_nama_instrumen,
        Assm_No_identitas_Istrumen,
...
async function updateTemperatureControlCertificateHeader({ qaId, idNoSertifikat, header, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SET
        parameter_sertifikasi = COALESCE(NULLIF(parameter_sertifikasi, ''), 'Temperature Control'),
...
async function replaceTemperatureControlCertificateRows({ qaId, idNoSertifikat, rows, userId, delegatedTo, transaction }) {
  await sequelizeMSQL.query(
    `
...
const listSessions = async (req, res, next) => {
  try {
    ...
    console.error('Error in TemperatureControl listSessions:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(404).json({
        success: false,
        message: 'Temperature Control session table has not been created',
      });
    }
    ...
    console.error('Error in TemperatureControl getSession:', error);
    next(error);
  }
};
...
const getPrintData = async (req, res, next) => {
  try {
    ...
    if (!headerResults.length) {
      return res.status(404).json({
        success: false,
        message: 'Temperature Control sertifikat data not found',
      });
    }

    const workbookPayload = latestSession?.workbookPayload || {};
    const rows = buildTemperatureControlResultRows(latestSession?.calculationResult || {});
    ...
    console.error('Error in TemperatureControl getPrintData:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(500).json({
        success: false,
        message: 'Temperature Control session table has not been created. Run sql/create-temperature-control-calibration-tables.sql first.',
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Temperature Control workbook session saved successfully',
      data: session,
    });
  } catch (error) {
    console.error('Error in TemperatureControl saveSession:', error);
    next(error);
  }
};
...
const approveSession = async (req, res, next) => {
  try {
    ...
    console.error('Error in TemperatureControl approveSession:', error);
    next(error);
  }
};

const generateTemperatureControlSertifikat = async (req, res, next) => {
  ...
  try {
    ...
    if (!qaId) {
      ...
      return res.status(400).json({
        success: false,
        message: 'QA_ID is required to generate Temperature Control certificate',
      });
    }

    if (!idNoSertifikat) {
      idNoSertifikat = await getNextTemperatureControlCertificateNumber(transaction);
    }
    ...
    if (!idNoSertifikat) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Gagal mengambil nomor otomatis sertifikat Temperature Control',
      });
    }

    const headerCreated = await ensureTemperatureControlCertificateHeader({
      qaId,
      idNoSertifikat,
      userId: user_id,
      delegatedTo: delegated_to,
    ...
    await updateTemperatureControlCertificateHeader({
      qaId,
      idNoSertifikat,
    ...
    const resultRows = buildTemperatureControlResultRows(
      calculationResult,
      body.certificateRows || []
    );
    await replaceTemperatureControlCertificateRows({
      qaId,
      idNoSertifikat,
      rows: resultRows,
      userId: user_id,
    ...
    return res.status(200).json({
      success: true,
      message: 'Sukses Generate Data Sertifikat Temperature Control!',
      data: {
        qa_id: qaId,
        id_no_sertifikat: idNoSertifikat,
    ...
    console.error('Error in generateTemperatureControlSertifikat:', error);
    next(error);
  }
};
...
module.exports = {
  ...
  saveSession,
  approveSession,
  generateTemperatureControlSertifikat,
};
