...
const SESSION_TABLE = 'dbo.T_Kalibrasi_LeakTest_Workbook_Session';
...
function buildLeakTestResultRows(calculationResult = {}, explicitRows = []) {
  ...
  return rows.map((row, index) => {
    const setting = parseNumberValue(row.setting ?? row.Setting);
    const avgUut = parseNumberValue(
      row.avgUut ?? row.pembacaanAlat ?? row.Pembacaan_Alat
    );
    const avgStd = parseNumberValue(
      row.avgStd ?? row.pembacaanStandar ?? row.Pembacaan_standar
    );
    const avgError = parseNumberValue(row.avgError ?? row.error ?? row.ERROR) ??
      (avgUut === null || avgStd === null ? null : avgUut - avgStd);

    return {
      seqId: Number(row.seqId || row.Seq_ID || index + 1),
      no: Number(row.no || row.seqId || index + 1),
      setting,
      pembacaanAlat: avgUut,
      pembacaanStandar: avgStd,
      error: avgError,
...
async function getNextLeakTestCertificateNumber(transaction) {
  const rows = await sequelizeMSQL.query(
    'SELECT dbo.fnGetKal_Ser_LT_No_ID() as ID_No_sertifikat',
    {
      type: Sequelize.QueryTypes.SELECT,
      transaction,
    }
...
async function ensureLeakTestCertificateHeader({ qaId, idNoSertifikat, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SELECT
        ...
        Jenis_kalibrasi,
        COALESCE(NULLIF(Parameter_Sertifikasi, ''), 'Leak Test') AS parameter_sertifikasi,
        1 AS isSert_Manual,
        GETDATE() AS Tgl,
        Assm_nama_instrumen,
        Assm_No_identitas_Istrumen,
...
async function updateLeakTestCertificateHeader({ qaId, idNoSertifikat, header, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SET
        parameter_sertifikasi = COALESCE(NULLIF(parameter_sertifikasi, ''), 'Leak Test'),
...
async function replaceLeakTestCertificateRows({ qaId, idNoSertifikat, rows, userId, delegatedTo, transaction }) {
  await sequelizeMSQL.query(
    `
...
const listSessions = async (req, res, next) => {
  try {
    ...
    console.error('Error in LeakTest listSessions:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(404).json({
        success: false,
        message: 'Leak Test session table has not been created',
      });
    }
    ...
    console.error('Error in LeakTest getSession:', error);
    next(error);
  }
};
...
const getPrintData = async (req, res, next) => {
  try {
    ...
    if (!headerResults.length) {
      return res.status(404).json({
        success: false,
        message: 'Leak Test sertifikat data not found',
      });
    }

    const workbookPayload = latestSession?.workbookPayload || {};
    const rows = buildLeakTestResultRows(latestSession?.calculationResult || {});
    ...
    console.error('Error in LeakTest getPrintData:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(500).json({
        success: false,
        message: 'Leak Test session table has not been created. Run sql/create-leak-test-calibration-tables.sql first.',
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Leak Test workbook session saved successfully',
      data: session,
    });
  } catch (error) {
    console.error('Error in LeakTest saveSession:', error);
    next(error);
  }
};
...
const approveSession = async (req, res, next) => {
  try {
    ...
    console.error('Error in LeakTest approveSession:', error);
    next(error);
  }
};

const generateLeakTestSertifikat = async (req, res, next) => {
  ...
  try {
    ...
    if (!qaId) {
      ...
      return res.status(400).json({
        success: false,
        message: 'QA_ID is required to generate Leak Test certificate',
      });
    }

    if (!idNoSertifikat) {
      idNoSertifikat = await getNextLeakTestCertificateNumber(transaction);
    }
    ...
    if (!idNoSertifikat) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Gagal mengambil nomor otomatis sertifikat Leak Test',
      });
    }

    const headerCreated = await ensureLeakTestCertificateHeader({
      qaId,
      idNoSertifikat,
      userId: user_id,
      delegatedTo: delegated_to,
    ...
    await updateLeakTestCertificateHeader({
      qaId,
      idNoSertifikat,
    ...
    const resultRows = buildLeakTestResultRows(
      calculationResult,
      body.certificateRows || []
    );
    await replaceLeakTestCertificateRows({
      qaId,
      idNoSertifikat,
      rows: resultRows,
      userId: user_id,
    ...
    return res.status(200).json({
      success: true,
      message: 'Sukses Generate Data Sertifikat Leak Test!',
      data: {
        qa_id: qaId,
        id_no_sertifikat: idNoSertifikat,
    ...
    console.error('Error in generateLeakTestSertifikat:', error);
    next(error);
  }
};
