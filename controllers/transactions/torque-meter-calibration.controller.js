...
const SESSION_TABLE = 'dbo.T_Kalibrasi_TorqueMeter_Workbook_Session';
...
function buildTorqueResultRows(calculationResult = {}) {
  const tables = Array.isArray(calculationResult?.tables)
    ? calculationResult.tables
    : [];

  return tables.map((table, index) => ({
    seqId: index + 1,
    pembacaanAlat: parseNumberValue(table.avgUut),
    pembacaanStandar: parseNumberValue(table.avgStandard),
    error: parseNumberValue(table.avgError),
    ketidakpastian: parseNumberValue(table.uExpanded),
  }));
}
...
async function ensureTorqueCertificateHeader({ qaId, idNoSertifikat, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SELECT
        ...
        Jenis_kalibrasi,
        COALESCE(NULLIF(Parameter_Sertifikasi, ''), 'Torque Meter') AS parameter_sertifikasi,
        1 AS isSert_Manual,
        GETDATE() AS Tgl,
        Assm_nama_instrumen,
        Assm_No_identitas_Istrumen,
...
async function updateTorqueCertificateHeader({ qaId, idNoSertifikat, header, userId, delegatedTo, transaction }) {
...
async function replaceTorqueCertificateRows({ qaId, idNoSertifikat, rows, userId, delegatedTo, transaction }) {
  await sequelizeMSQL.query(
    `
...
const listSessions = async (req, res, next) => {
  try {
    ...
    console.error('Error in torque-meter listSessions:', error);
    next(error);
  }
};
...
const getSession = async (req, res, next) => {
  try {
    ...
    console.error('Error in torque-meter getSession:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(500).json({
        success: false,
        message: 'Torque meter session table has not been created. Run sql/create-torque-meter-calibration-tables.sql first.',
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Torque meter workbook session saved successfully',
      data: session,
    });
  } catch (error) {
...
const generateTorqueSertifikat = async (req, res, next) => {
  ...
  try {
    ...
    if (!qaId) {
      ...
      return res.status(400).json({
        success: false,
        message: 'QA_ID is required to generate Torque Meter certificate',
      });
    }

    if (!idNoSertifikat) {
      idNoSertifikat = await getNextTorqueCertificateNumber(transaction);
    }
    ...
    if (!idNoSertifikat) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Gagal mengambil nomor otomatis sertifikat Torque Meter',
      });
    }

    const headerCreated = await ensureTorqueCertificateHeader({
      qaId,
      idNoSertifikat,
      userId: user_id,
      delegatedTo: delegated_to,
    ...
    await updateTorqueCertificateHeader({
      qaId,
      idNoSertifikat,
    ...
    return res.status(200).json({
      success: true,
      message: 'Sukses Generate Data Sertifikat Torque Meter!',
      data: {
        qa_id: qaId,
        id_no_sertifikat: idNoSertifikat,
    ...
    console.error('Error in generateTorqueSertifikat:', error);
    next(error);
  }
};
