 ...
 * HTTP handlers for the Timer (Stopwatch) Calibration module.
 * All business logic lives in src/services/timerCalculation.service.js.
 */
...
function sendError(res, error) {
  ...
  console.error('[timer-calibration] unexpected error:', error);
...
async function listSessions(req, res) {
  try {
    const data = await repo.listSessions({ status: req.query.status, unitMode: req.query.unitMode });
...
async function saveWorkbook(req, res) {
  try {
    ...
    const result = await calc.saveWorkbook(sessionId, points);
    return res.status(200).json({ success: true, data: result });
  } catch (error) {
    return sendError(res, error);
  }
