...
const SESSION_TABLE = 'dbo.T_Kalibrasi_Enclosures_Workbook_Session';
...
function buildEnclosuresResultRows(calculationResult = {}, explicitRows = []) {
  const rows = Array.isArray(explicitRows) && explicitRows.length
    ? explicitRows
    : [
        ...(Array.isArray(calculationResult?.temperature)
          ? calculationResult.temperature
          : []),
        ...(Array.isArray(calculationResult?.humidity)
  ...
  return rows.map((row, index) => {
    ...
    const avgUut = parseNumberValue(
      row.avgUut ?? row.indicatorValue ?? row.pembacaanAlat ?? row.Pembacaan_Alat
    );
    const avgStd = parseNumberValue(
      row.avgStd ?? row.measurementValue ?? row.pembacaanStandar ?? row.Pembacaan_standar
    );
    ...
    return {
      ...
      ketidakpastian: parseNumberValue(
        row.ketidakpastian ?? row.uncertainty?.expanded ?? row.Ketidakpastian
      ),
    };
  });
}
...
async function getNextEnclosuresCertificateNumber(transaction) {
  const rows = await sequelizeMSQL.query(
    'SELECT dbo.fnGetKal_Ser_E_No_ID() as ID_No_sertifikat',
    {
      type: Sequelize.QueryTypes.SELECT,
      transaction,
    }
...
async function ensureEnclosuresCertificateHeader({ qaId, idNoSertifikat, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SELECT
        ...
        Jenis_kalibrasi,
        COALESCE(NULLIF(Parameter_Sertifikasi, ''), 'Enclosures') AS parameter_sertifikasi,
        1 AS isSert_Manual,
        GETDATE() AS Tgl,
        Assm_nama_instrumen,
        Assm_No_identitas_Istrumen,
...
async function updateEnclosuresCertificateHeader({ qaId, idNoSertifikat, header, userId, delegatedTo, transaction }) {
  ...
  await sequelizeMSQL.query(
    `
      ...
      SET
        parameter_sertifikasi = COALESCE(NULLIF(parameter_sertifikasi, ''), 'Enclosures'),
...
async function replaceEnclosuresCertificateRows({ qaId, idNoSertifikat, rows, userId, delegatedTo, transaction }) {
  await sequelizeMSQL.query(
    `
...
const listSessions = async (req, res, next) => {
  try {
    ...
    console.error('Error in Enclosures listSessions:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(404).json({
        success: false,
        message: 'Enclosures session table has not been created',
      });
    }
    ...
    console.error('Error in Enclosures getSession:', error);
    next(error);
  }
};
...
const getPrintData = async (req, res, next) => {
  try {
    ...
    if (!headerResults.length) {
      return res.status(404).json({
        success: false,
        message: 'Enclosures sertifikat data not found',
      });
    }

    const workbookPayload = latestSession?.workbookPayload || {};
    const rows = buildEnclosuresResultRows(latestSession?.calculationResult || {});
    ...
    console.error('Error in Enclosures getPrintData:', error);
    next(error);
  }
};
  ...
  try {
    ...
    if (!(await sessionTableExists())) {
      return res.status(500).json({
        success: false,
        message: 'Enclosures session table has not been created. Run sql/create-enclosures-calibration-tables.sql first.',
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Enclosures workbook session saved successfully',
      data: session,
    });
  } catch (error) {
    console.error('Error in Enclosures saveSession:', error);
    next(error);
  }
};
...
const generateEnclosuresSertifikat = async (req, res, next) => {
