...
const checkStatusLaporanTrialSkalaLab = async (id) => {
  let apprNo = 1;
  const checkStatus = await t_laporanTrialSkalaLab_status.findAll({
    where: {
...
const checkStatusProposalDiversifikasi = async (id) => {
  let apprNo = 1;
  const checkStatus = await t_proposalDiversifikasi_status.findAll({
    where: {
