...
const SESSION_COLUMNS = `
  ...
  pic, conclusion, created_by, updated_by, created_at, updated_at
`;
...
async function listSessions(filters = {}) {
  ...
  let query = `
    ...
    FROM [dbo].[temperature_sessions] s
  `;
...
async function getSessionById(sessionId, transaction) {
  ...
  const result = await request
    ...
    .query(`SELECT ${SESSION_COLUMNS} FROM [dbo].[temperature_sessions] WHERE session_id = @SessionId`);
...
async function createSession(payload, transaction) {
  ...
  const result = await request.query(`
    INSERT INTO [dbo].[temperature_sessions]
    (
      session_code, instrument_id, instrument_code, instrument_name,
      merk_tipe, no_seri, kapasitas, resolusi, lokasi, calibration_date,
      interval_bulan, metode_kalibrasi, keterangan, tolerance_accuracy,
      temperature, humidity, std_nama, std_no_identitas, std_no_sertifikat,
      std_tertelusur, std_rekalibrasi, qa_id, status, pic, created_by
    )
    VALUES
    (
      ...
      @StdTertelusur, @StdRekalibrasi, @QaId, @Status, @Pic, @CreatedBy
    );
...
async function updateSession(sessionId, payload, transaction) {
  ...
  const result = await request.query(`
    UPDATE [dbo].[temperature_sessions] SET
...
async function updateSessionStatus(sessionId, status, updatedBy, transaction) {
  ...
  await request
    ...
    .query(`
      UPDATE [dbo].[temperature_sessions]
...
async function updateSessionConclusion(sessionId, conclusion, transaction) {
  ...
  await request
    ...
    .query('UPDATE [dbo].[temperature_sessions] SET conclusion = @Conclusion WHERE session_id = @SessionId');
}
...
async function deleteWorkbookBySession(sessionId, transaction) {
  ...
  await request
    .input('SessionId', sql.Int, sessionId)
    .query(`
      DELETE FROM [dbo].[temperature_results] WHERE session_id = @SessionId;
      DELETE FROM [dbo].[temperature_result_summary] WHERE session_id = @SessionId;
      DELETE FROM [dbo].[temperature_readings] WHERE session_id = @SessionId;
      DELETE FROM [dbo].[temperature_points] WHERE session_id = @SessionId;
    `);
}
...
async function deleteSessionGraph(sessionId, transaction) {
  ...
  const result = await request
    .input('SessionId', sql.Int, sessionId)
    .query('DELETE FROM [dbo].[temperature_sessions] WHERE session_id = @SessionId');
  return (result.rowsAffected && result.rowsAffected[0] > 0) || false;
}
...
async function insertPoint(sessionId, point, transaction) {
  ...
  await request
    ...
    .input('CorrectionStandard', sql.Decimal(18, 10), point.correction_standard || 0)
    .input('CorrectionMedia', sql.Decimal(18, 10), point.correction_media || 0)
    .input('CorrectionUut', sql.Decimal(18, 10), point.correction_uut || 0)
    .input('Uc1', sql.Decimal(18, 10), point.uc_1 || 0)
    .input('Uc2', sql.Decimal(18, 10), point.uc_2 || 0)
    .input('Uc3', sql.Decimal(18, 10), point.uc_3 || 0)
    ...
    .query(`
      INSERT INTO [dbo].[temperature_points]
        (session_id, point_no, nominal_value, correction_standard, correction_media,
         correction_uut, uc_1, uc_2, uc_3, digital_resolution, analog_resolution)
      VALUES
        (@SessionId, @PointNo, @NominalValue, @CorrectionStandard, @CorrectionMedia,
         @CorrectionUut, @Uc1, @Uc2, @Uc3, @DigitalResolution, @AnalogResolution)
    `);
}
...
async function insertReading(sessionId, row, transaction) {
  ...
  await request
    ...
    .query(`
      INSERT INTO [dbo].[temperature_readings]
...
async function listPoints(sessionId, transaction) {
  ...
  const result = await request
    .input('SessionId', sql.Int, sessionId)
    .query('SELECT * FROM [dbo].[temperature_points] WHERE session_id = @SessionId ORDER BY point_no');
  return result.recordset;
}
...
async function listReadings(sessionId, transaction) {
  ...
  const result = await request
    .input('SessionId', sql.Int, sessionId)
    .query('SELECT * FROM [dbo].[temperature_readings] WHERE session_id = @SessionId ORDER BY point_no, sequence_no');
  return result.recordset;
}
...
async function insertResult(row, transaction) {
  ...
  await request
    ...
    .query(`
      INSERT INTO [dbo].[temperature_results]
...
async function upsertSummary(sessionId, summary, transaction) {
  ...
  await request
    ...
    .query(`
      IF EXISTS (SELECT 1 FROM [dbo].[temperature_result_summary] WHERE session_id = @SessionId)
        UPDATE [dbo].[temperature_result_summary]
        ...
        WHERE session_id = @SessionId
      ELSE
        INSERT INTO [dbo].[temperature_result_summary] (session_id, coverage_factor, max_u_expanded, conclusion)
...
async function getResults(sessionId, transaction) {
  ...
  const result = await request
    .input('SessionId', sql.Int, sessionId)
    .query('SELECT * FROM [dbo].[temperature_results] WHERE session_id = @SessionId ORDER BY point_no');
  return result.recordset;
}
...
async function getSummary(sessionId, transaction) {
  ...
  const result = await request
    .input('SessionId', sql.Int, sessionId)
    .query('SELECT * FROM [dbo].[temperature_result_summary] WHERE session_id = @SessionId');
  return result.recordset[0] || null;
}

async function listTemperatureStandards(transaction) {
  const request = await createRequest(transaction);
  const result = await request.query('SELECT * FROM [dbo].[temperature_standards] ORDER BY nominal_value');
  return result.recordset;
}

async function listTemperatureDaCandidates(filters = {}, transaction) {
  const request = await createRequest(transaction);
  const where = [];
  if (filters.search) {
...
async function createSertifikatBagianDraftFromTemperatureDa(qaId, idNoSertifikat, userId, delegatedTo, transaction) {
  const request = await createRequest(transaction);
  const result = await request
...
module.exports = {
  ...
  getResults,
  getSummary,
  listTemperatureStandards,
  listTemperatureDaCandidates,
  createSertifikatBagianDraftFromTemperatureDa,
