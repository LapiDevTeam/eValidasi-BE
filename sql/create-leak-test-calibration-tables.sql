IF OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session', 'U') IS NULL
BEGIN
  CREATE TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session
  (
    ...
    Status NVARCHAR(20) NOT NULL CONSTRAINT DF_LeakTestWorkbook_Status DEFAULT (N'DRAFT'),
    ...
    Process_Date DATETIME NOT NULL CONSTRAINT DF_LeakTestWorkbook_ProcessDate DEFAULT (GETDATE()),
    Update_Date DATETIME NULL
  )
END
GO

IF OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session', 'U') IS NOT NULL
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM sys.columns
    WHERE object_id = OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session')
      AND name = 'QA_ID'
  )
  BEGIN
    ALTER TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session
  ...
  BEGIN
    ...
    EXEC(N'UPDATE dbo.T_Kalibrasi_LeakTest_Workbook_Session
    SET Status = N''DRAFT''
    WHERE Status IS NULL')

    EXEC(N'ALTER TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session
  ...
  IF NOT EXISTS (
    ...
    WHERE dc.parent_object_id = OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session')
      AND c.name = 'Status'
  )
  BEGIN
    EXEC(N'ALTER TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session
    ADD CONSTRAINT DF_LeakTestWorkbook_Status DEFAULT (N''DRAFT'') FOR Status')
  END
  ...
  BEGIN
    EXEC(N'ALTER TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session
    ADD CONSTRAINT DF_LeakTestWorkbook_ProcessDate DEFAULT (GETDATE()) FOR Process_Date')
  END
...
IF NOT EXISTS (
  SELECT 1
  FROM sys.indexes
  WHERE name = 'IX_LeakTestWorkbook_QA_Sertifikat'
    AND object_id = OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session')
)
BEGIN
  CREATE INDEX IX_LeakTestWorkbook_QA_Sertifikat
  ON dbo.T_Kalibrasi_LeakTest_Workbook_Session
  (
    QA_ID,
    ID_No_Sertifikat,
    Session_ID DESC
...
IF OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist', 'U') IS NULL
BEGIN
  CREATE TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist
  (
    ...
    Change_Date DATETIME NOT NULL CONSTRAINT DF_LeakTestWorkbookHist_ChangeDate DEFAULT (GETDATE())
  )
END
GO

IF OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist', 'U') IS NOT NULL
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM sys.columns
    WHERE object_id = OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist')
      AND name = 'Session_ID'
  )
  BEGIN
    ALTER TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist
  ...
  BEGIN
    ...
    EXEC(N'UPDATE dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist
    SET Change_Date = GETDATE()
    WHERE Change_Date IS NULL')

    EXEC(N'ALTER TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist
  ...
  IF NOT EXISTS (
    ...
    INNER JOIN sys.columns AS c
      ON c.object_id = dc.parent_object_id
     AND c.column_id = dc.parent_column_id
    WHERE dc.parent_object_id = OBJECT_ID('dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist')
      AND c.name = 'Change_Date'
  )
  BEGIN
    EXEC(N'ALTER TABLE dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist
    ADD CONSTRAINT DF_LeakTestWorkbookHist_ChangeDate DEFAULT (GETDATE()) FOR Change_Date')
  END
END
GO

IF OBJECT_ID('dbo.TR_T_Kalibrasi_LeakTest_Workbook_Session_Hist', 'TR') IS NOT NULL
BEGIN
  DROP TRIGGER dbo.TR_T_Kalibrasi_LeakTest_Workbook_Session_Hist
END
GO

CREATE TRIGGER dbo.TR_T_Kalibrasi_LeakTest_Workbook_Session_Hist
ON dbo.T_Kalibrasi_LeakTest_Workbook_Session
...
BEGIN
  ...
  INSERT INTO dbo.T_Kalibrasi_LeakTest_Workbook_Session_Hist
  (
    Session_ID,
    QA_ID,
    ID_No_Sertifikat,
