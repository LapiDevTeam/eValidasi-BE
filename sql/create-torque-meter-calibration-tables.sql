IF OBJECT_ID('dbo.T_Kalibrasi_TorqueMeter_Workbook_Session', 'U') IS NULL
BEGIN
  CREATE TABLE dbo.T_Kalibrasi_TorqueMeter_Workbook_Session
  (
...
IF NOT EXISTS (
  SELECT 1
  FROM sys.indexes
  WHERE name = 'IX_TorqueWorkbook_QA_Sertifikat'
    AND object_id = OBJECT_ID('dbo.T_Kalibrasi_TorqueMeter_Workbook_Session')
)
BEGIN
  CREATE INDEX IX_TorqueWorkbook_QA_Sertifikat
  ON dbo.T_Kalibrasi_TorqueMeter_Workbook_Session
  (
    QA_ID,
    ID_No_Sertifikat,
    Session_ID DESC
...
IF OBJECT_ID('dbo.T_Kalibrasi_TorqueMeter_Workbook_Session_Hist', 'U') IS NULL
BEGIN
  CREATE TABLE dbo.T_Kalibrasi_TorqueMeter_Workbook_Session_Hist
  (
...
IF OBJECT_ID('dbo.TR_T_Kalibrasi_TorqueMeter_Workbook_Session_Hist', 'TR') IS NOT NULL
BEGIN
  DROP TRIGGER dbo.TR_T_Kalibrasi_TorqueMeter_Workbook_Session_Hist
END
GO

CREATE TRIGGER dbo.TR_T_Kalibrasi_TorqueMeter_Workbook_Session_Hist
ON dbo.T_Kalibrasi_TorqueMeter_Workbook_Session
...
BEGIN
  ...
  INSERT INTO dbo.T_Kalibrasi_TorqueMeter_Workbook_Session_Hist
  (
    Session_ID,
    QA_ID,
    ID_No_Sertifikat,
